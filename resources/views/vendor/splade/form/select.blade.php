            ...
            <div class="relative">
                <div v-bind:class="{ 'opacity-50': select.loading }">
                    <select {{ $attributes->except(['v-if', 'v-show', 'class'])->class([
                        'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 disabled:opacity-50 dark:bg-gray-800 dark:text-gray-200 dark:border-gray-600 dark:focus:border-indigo-400 dark:focus:ring-indigo-300 placeholder:text-gray-300',
                    ])->merge([
                        'multiple' => $multiple,
                        'name' => $name,
                ...
                <div class="absolute inset-0 w-full h-full" v-if="select.loading">
                    <div class="flex flex-row items-center justify-center w-full h-full">
                        <svg
                            class="animate-spin ltr:mr-3 rtl:ml-3 h-5 w-5 text-indigo-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        >
